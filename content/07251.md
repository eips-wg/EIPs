...
The [EIP-7685](./07685.md) encoding of a withdrawal request **MUST** be computed as follows:
...
2. Check that consolidation requests in the [EIP-7685](./07685.md) requests list matches the list returned from `dequeue_consolidation_requests()` function of the smart contract respecting the order of the returned requests. If this condition does not hold, the block **MUST** be deemed *invalid*.
...
3. ***Adding execution layer partial withdrawals (part of [EIP-7002](./07002.md)).***
