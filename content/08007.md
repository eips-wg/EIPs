...
|:---:|---|---|:---:|---|
| [EIP-2780](./02780.md) | Reduce intrinsic transaction gas and charge 25k when a value transfer creates a new account. | Broad harmonization | Multi-resource | Decrease `TX_BASE_COST` (21k→4.5k); increase new-account surcharge (0→25k) |
| [EIP-7778](./07778.md) | Prevent Block Gas Limit Circumvention by Excluding Refunds from Block Gas Accounting. | Pricing extension | General Accounting | No opcode repricing; excludes gas refunds (e.g. `SSTORE` clearing) from block gas accounting |
| [EIP-7904](./07904.md) | Gas Cost Increase to reflect computational complexity and transaction throughput increase | Broad harmonization | Compute | All increase: `DIV`, `SDIV`, `MOD`, `MULMOD`, `KECCAK256`, precompiles (`BLAKE2F`, `BLS12_G1ADD`, `BLS12_G2ADD`, `ECADD`, `ECPAIRING`, `POINT_EVALUATION`) |
| [EIP-7976](./07976.md) | Further increase calldata cost to 15/60 gas per byte to reduce maximum block size. | Pricing extension | Data | Increase calldata floor cost (10/40 → 15/60 per byte) |
| [EIP-7981](./07981.md) | Introduce floor pricing for access lists to reduce maximum block size. | Pricing extension | Data | Increase: new floor charge on access list data (addresses + storage keys) |
| [EIP-8037](./08037.md) | Harmonization, increase and separate metering of state creation gas costs to mitigate state growth and unblock scaling. | Broad harmonization | State | All increase (dynamic with gas limit): `GAS_CREATE`, `GAS_CODE_DEPOSIT`, `GAS_NEW_ACCOUNT`, `GAS_STORAGE_SET`, `PER_AUTH_BASE_COST` |
| [EIP-8038](./08038.md) | Increases the gas cost of state-access operations to reflect Ethereum's larger state. | Broad harmonization | State | All increase: cold storage write/access, cold account access, warm access, `EXTCODESIZE`/`EXTCODECOPY` extra read, access list costs |

### Declined for Inclusion

| EIP | Description | Type | Resource class |
|:---:|---|---|:---:|
| [EIP-2926](./02926.md) | Introduce code-chunking in an MPT context. | Pricing extension | State |
| [EIP-7686](./07686.md) | Adjust memory limits and gas limits of sub-calls to create a clear linear bound on how much total memory an EVM execution can consume. | Pricing extension | Memory |
| [EIP-7923](./07923.md) | Linearize Memory Costing and replace the current quadratic formula with a page-based cost model. | Pricing extension | Memory |
| [EIP-7971](./07971.md) | Decrease costs for `TLOAD` and `TSTORE` with a transaction-global limit. | Pricing extension | Memory |
| [EIP-7973](./07973.md) | Introduce warm account writes, decreasing the cost of writing to an account after the first write. | Pricing extension | State |
| [EIP-8011](./08011.md) | Gas accounting by EVM resource, increasing throughput and improving resource usage controls, with minimal changes to the protocol and UX. | Supporting | NA |
| [EIP-8032](./08032.md) | Makes `SSTORE` gas cost scale with a contract's storage size to discourage state bloat. | Pricing extension | State |
| [EIP-8053](./08053.md) | Adds `milli-gas` as the EVM's internal gas accounting unit, reducing rounding errors without impacting UX. | Supporting | NA |
| [EIP-8057](./08057.md) | Multi‑block temporal locality discounts for state and account access. | Pricing extension | State |
| [EIP-8058](./08058.md) | Reduces gas costs for deploying duplicate contract bytecode via access-list based mechanism. | Pricing extension | State |
| [EIP-8059](./08059.md) | Gas parameters and variables are increased to a factor of `REBASE_FACTOR` to reduce rounding errors without major changes to the EVM. | Supporting | NA |
...
#### Compute Opcodes — [EIP-7904](./07904.md) (all increase)
...
#### Precompiles — [EIP-7904](./07904.md) (all increase)
...
#### State Access — [EIP-8038](./08038.md) (all increase, values TBD)
...
#### State Creation — [EIP-8037](./08037.md) (all increase, dynamic with gas limit)
...
| --- | ---: | --- | --- | --- |
| `TX_BASE_COST` | 21,000 | 4,500 | decrease | [2780](./02780.md) |
| New account surcharge (top-level value tx) | 0 | 25,000 | increase | [2780](./02780.md) |
| Calldata floor cost | 10/40 per byte | 15/60 per byte | increase | [7976](./07976.md) |
| Access list data cost | 0 | `floor_token_cost` per token | increase (new) | [7981](./07981.md) |
