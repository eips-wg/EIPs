...
2. Replace `SSTORE_CLEARS_SCHEDULE` (as defined in [EIP-2200](./02200.md)) with `SSTORE_RESET_GAS + ACCESS_LIST_STORAGE_KEY_COST` (4,800 gas as of [EIP-2929](./02929.md) + [EIP-2930](./02930.md))
...
In [EIP-2200](./02200.md#specification), three cases for refunds were introduced:
...
A criticism of earlier refund removal EIPs ([EIP-3298](./03298.md) and [EIP-3403](./03403.md)) is that these EIPs fully remove the incentive to set a value to zero, encouraging users to not fully clear a storage slot if they expect even the smallest probability that they will want to use that storage slot again.
...
Note, there is a difference between 'hot' and 'cold' slots. This table shows the values as of [EIP-2929](./02929.md) assuming that all touched storage slots were already 'hot' (the difference being a one-time cost of `2100` gas).
