...
- Using the [progressive Merkle tree](./07916.md) structure to progressively grow to the actual field count with minimal overhead, ensuring provers remain valid as the field count changes.
...
Light client based verifiers and smart contracts (e.g., based on [EIP-4788](./04788.md)) do not update at the same cadence as Ethereum. If a future fork removes a field from a `ProgressiveContainer(active_fields)`, the `active_fields` mix-in enables such verifiers to distinguish absent fields from `0` values. Without `active_fields`, the `hash_tree_root` for these cases would collide.
