...
Extending the format introduced in [EIP-3540](./03540.md), this EIP proposes to add a new OPTIONAL section in the body called `metadata_section` before the `data_section`, and to add two new OPTIONAL fields `kind_metadata` (value: `0x05`) and `metadata_size` to the header before the `kind_data` and `data_size` fields.
...
No backward compatibility issues are expected since [EIP-3540](./03540.md) is not implemented yet.
