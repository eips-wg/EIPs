...
2. **Payload-size bounds:** Per [EIP-7934](../07934.md) the execution payload cap is `MAX_RLP_BLOCK_SIZE = 10,485,760 - 2,097,152 = 8,388,608` bytes. Payload size, not gas, may become the dominant block-size constraint at high gaslimits.
...
The intrinsic gas accounting defined here overrides [EIP-2929](../02929.md)'s blanket "all tx addresses are warm" rule.
...
This decomposition replaces the implicit warmth assumptions of [EIP-2929](../02929.md) and makes the per-account cost explicit in the base or the execution phase as appropriate.
...
4. `to` is non-existent per [EIP-161](../00161.md) emptiness at the start of transaction execution.
...
- If `value = 0` and `to` is empty per [EIP-161](../00161.md), no account is created and no surcharge applies.
...
- **Warmth policy.**
  ...
  - Access-list entries (charged via [EIP-2930](../02930.md)) and precompiles are warm at tx start.
...
- **[EIP-2930](../02930.md) (Access List).** Intrinsic gas is `TX_BASE_COST` after `FORK_BLOCK`. Access lists keep their existing per-entry charges and warming semantics.
- **[EIP-2929](../02929.md) (Gas cost increases for state access).** Refined by this EIP to price non-code cold touches at `500` and code-account cold touches at `2,600`.
- **[EIP-7702](../07702.md) (Set EOA Code).** `TX_BASE_COST` remains `4,500` even if the sender temporarily assumes code for the transaction. Clients must not perform any disk code-load to determine sender type, since 7702 provides code inline. If the transaction executes code that reads or executes its own code, normal `COLD_ACCOUNT_COST_CODE` or `WARM_STATE_READ` costs apply at execution time as per the standard cold/warm model.
- **Calldata-pricing EIPs (e.g. [EIP-7623](../07623.md)).** Unchanged. This EIP does not alter calldata pricing.
...
This EIP composes cleanly with calldata-pricing proposals such as [EIP-7623](../07623.md) and [EIP-7976](../07976.md). Those EIPs affect `tx.data` metering, while this EIP adjusts only transaction-level intrinsic accounting. There is no overlapping scope.
...
- **Serialization neutrality.** A fee rule keyed to RLP size couples costs to one encoding and weakens [EIP-2718](../02718.md) type neutrality and future formats such as SSZ. Calldata is treated as opaque bytes so it avoids this coupling.

- **Policy targeting and market floor.** Calldata floors (for example [EIP-7623](../07623.md) or [EIP-7976](../07976.md)) price bytes in `tx.data` to bound EL payload and steer data to blobs; envelope bytes are control-plane. The `TX_BASE_COST` is the smallest tx size, and having a low encoding-agnostic intrinsic base keeps the minimum inclusion price coupled to the basefee market, without opening a DoS vector.
...
For execution-payload size, use [EIP-7934](../07934.md)’s 8 MiB uncompressed cap: `8,388,608 bytes`. A typical EIP-1559 ETH transfer encodes at roughly `~110 bytes`, yielding a size cap of `floor(8,388,608 / 110) = 76,260 tx` per block when size, not gas, is the binding constraint.
...
13. Perfnet stress
    - Fill block with minimal 4,500-gas tx and 6,000-gas ETH transfers; confirm gas vs byte-size binding per [EIP-7934](../07934.md).
...
The semantics of `SELFDESTRUCT` remain unchanged. Following [EIP-6780](../06780.md), only contracts created within the same transaction may be fully deleted. This EIP does not reprice or modify any `SELFDESTRUCT` side-effects.
