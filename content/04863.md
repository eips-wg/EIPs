...
Introduce a new [EIP-2718 transaction type](./02718.md) to support validator withdrawals that are "pushed" from the beacon chain to the EVM.
...
This approach is more involved than "pull"-based alternatives (e.g. [EIP-4788](./04788.md) + user-space withdrawal contract) with respect to the core protocol (by providing a new transaction type with special semantics) but does provide tighter integration of a critical feature into the protocol itself.
...
Define a new [EIP-2718](./02718.md) transaction type with `TransactionType` `WITHDRAWAL_TX_TYPE`.
