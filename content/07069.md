...
Introduce three new call instructions, `EXTCALL`, `EXTDELEGATECALL` and `EXTSTATICCALL`, with simplified semantics. Introduce another instruction, `RETURNDATALOAD` for loading a word from return data into stack. Modify the behavior of `RETURNDATACOPY` instruction executed within EOF formatted code (as defined by [EIP-3540](./03540.md)). The existing `*CALL` instructions are rejected by EOF validation.
...
Execution semantics of `RETURNDATACOPY` in EOF formatted code ([EIP-3540](./03540.md)) is modified as follows:
