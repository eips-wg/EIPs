...
This mechanism complements `EOFCREATE` and `RETURNCONTRACT` instructions from [EIP-7620](./07620.md), and thus all use cases of contract creation that are available in legacy EVM are enabled for EOF.
...
- fails (returns 0 on the stack) if container was invalid
    - caller’s nonce is not updated and gas for initcode execution is not consumed. Only `TX_CREATE_COST` constant and [EIP-3860](./03860.md) gas are consumed.
...
- behavior on `accessed_addresses` and address collision is same as `CREATE2` (rules for `CREATE2` from [EIP-684](./00684.md) and [EIP-2929](./02929.md) apply to `TXCREATE`)
...
- a successful execution ends with initcode executing `RETURNCONTRACT{deploy_container_index}(aux_data_offset, aux_data_size)` instruction (see [EIP-7620](./07620.md)). After that:
