...
This EIP reserves a [transaction-type](./02718.md) range for use by the Rollup Improvement Proposal (RIP) process to ensure there are no conflicts.
...
The transaction-type (as specified in [EIP-2718](./02718.md)) range from `0x40` to `0x7f` (inclusive of both) is reserved for use by the RIP process.
