...
Add a precompile that verifies multiple KZG openings `(z_i, y_i)` for a single [EIP-4844](./04844.md) blob commitment, returning the same 64-byte output as the existing point-evaluation precompile.

## Motivation

[EIP-4844](./04844.md) provides a point-evaluation precompile at `0x0A` costing 50,000 gas per opening. Contracts needing multiple blob values (e.g., fraud proofs) must pay this cost repeatedly. A batch interface verifies *k* openings in one call, reducing overhead.
...
On success, return 64 bytes identical to [EIP-4844](./04844.md):
...
- Preserves [EIP-4844](./04844.md) versioned-hash model and return format.
...
No changes to [EIP-4844](./04844.md) transactions, `BLOBHASH`, or the existing precompile at `0x0A`.

## Security Considerations


- Reject non-canonical field elements (`>= BLS_MODULUS`).
- Implementations MUST use the same trusted setup and subgroup checks as [EIP-4844](./04844.md).
