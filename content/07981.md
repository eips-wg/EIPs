...
This EIP charges access lists for their data footprint, preventing the circumvention of the [EIP-7623](./07623.md) floor pricing. This effectively reduces the worst-case block size by ~21% with minimal impact on users.

## Motivation

Access lists are only priced for storage but not for their data.

Furthermore, access lists can circumvent the [EIP-7623](./07623.md) floor pricing by contributing to EVM gas while still leaving a non-negligible data footprint. This enables achieving the maximal possible block size by combining access lists with calldata at a certain ratio.
...
The access list cost formula changes from [EIP-2930](./02930.md) to include data cost:
...
The [EIP-7623](./07623.md) floor calculation is modified to include access list tokens:
...
This EIP closes a loophole that allows circumventing [EIP-7623](./07623.md) floor pricing. Without this fix, attackers can achieve larger blocks than intended by combining access lists with calldata. All transaction data sources now contribute to the floor calculation consistently.
