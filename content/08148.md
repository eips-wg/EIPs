...
| Name                                                  | Value                                        | Comment                                                                              |
| ----------------------------------------------------- | -------------------------------------------- | ------------------------------------------------------------------------------------ |
| `SET_SWEEP_THRESHOLD_REQUEST_TYPE`                    | `0x03`                                       | The [EIP-7685](./07685.md) type prefix for set sweep threshold request            |
...
The new set sweep threshold request is an [EIP-7685](./07685.md) request with type `SET_SWEEP_THRESHOLD_REQUEST_TYPE` consisting of the following fields:

1. `source_address`: `Bytes20`
2. `validator_pubkey`: `Bytes48`
3. `threshold`: `uint64`

The [EIP-7685](./07685.md) encoding of a set sweep threshold request is computed as follows.
...
In response to the system call, the contract returns an opaque byte array of concatenated SSZ-serialized dequeued requests. There's no specific reasoning behind it, except aligning with the existing behaviour of the similar EIP, see [EIP-7002](./07002.md), and possible simplification of the processing flow for client teams.
...
* The call has a dedicated gas limit of `30_000_000` (`SYSTEM_TRANSACTION_GAS`) and is not subject to the transaction limit cap introduced in [EIP-7825](./07825.md).
...
Most of the considerations regarding the messaging format, queue, and rate-limiting are similar to those discussed in [EIP-7002](./07002.md) for withdrawal requests, and so we refer the reader to that EIP for more details.
...
Most of the security considerations regarding fee overpayment, system call failure, and empty code failure are similar to those discussed in [EIP-7002](./07002.md) for withdrawal requests, and so we refer the reader to that EIP for more details.
