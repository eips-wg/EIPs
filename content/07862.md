...
State root computation is a significant bottleneck in block production. With [EIP-7732](./07732.md) (ePBS), builders have tighter timing constraints. [EIP-7928](./07928.md) (Block-Level Access Lists) enables parallel state access but doesn't help with state root computation for builders.
...
With [EIP-7732](./07732.md), the `ExecutionPayloadEnvelope` contains a CL `state_root` verified at the end of `process_execution_payload`. This EIP changes only the EL header's `state_root` semantics. The CL state root verification is unaffected.

### BAL Synergy

[EIP-7928](./07928.md) provides the block access list, which identifies all storage slots touched during execution. With delayed state roots, clients can:
