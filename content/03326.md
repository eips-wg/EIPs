...
- `chainId`
  - **MUST** specify the integer ID of the chain as a hexadecimal string, per the [`eth_chainId`](./00695.md) Ethereum RPC method.
...
For related work, see [EIP-3085: `wallet_addEthereumChain`](./03085.md) and [EIP-2015: `wallet_updateEthereumChain`](./02015.md).
...
- Display a confirmation whenever a `wallet_switchEthereumChain` is received, clearly identifying the requester and the chain that will be switched to.
  - The confirmation used in [EIP-1102](./01102.md) may serve as a point of reference.
