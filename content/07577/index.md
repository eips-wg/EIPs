...
This EIP introduces a versioning scheme for [Standards Track](../00001.md#eip-types) EIPs by applying [Semantic Versioning 2.0.0](./assets/semver.md) based on changes made to the EIP's Specification section once its status has changed from `Draft` to `Review`.
...
The EIP versioning scheme MUST apply the following semantic versioning scheme of `MAJOR.MINOR.PATCH`, based on [Semantic Versioning 2.0.0](./assets/semver.md):
...
The history of [EIP-4788](../04788.md) contains many changes to its specification. EIP-4788 was updated to status "Review" on 2023-11-28. This case study assumes, however, that the EIP moved to status "Review" as of 2023-04-11 and updated to version 1.0.0 due to the start of a client team implementation.
