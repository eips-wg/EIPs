...
Transaction execution cannot be parallelized without knowing in advance which addresses and storage slots will be accessed. While [EIP-2930](../02930.md) introduced optional transaction access lists, they are not enforced.
...
Entries from an [EIP-2930](../02930.md) access list **MUST NOT** be included automatically. Only addresses and storage slots that are actually touched or changed during execution are recorded.
...
- **Post-state validation**: Gas costs requiring state access (account existence, [EIP-7702](../07702.md) delegation resolution)
...
The following table specifies pre-state validation costs in addition to the base opcode cost (gas constants as defined in [EIP-2929](../02929.md)):
...
- **Withdrawal recipients** (system withdrawals, [EIP-4895](../04895.md)).
...
Track **post‑transaction runtime bytecode** for deployed or modified contracts, and **delegation indicators** for successful delegations as defined in [EIP-7702](../07702.md).
...
- Deployed contracts.
- [EIP-7702](../07702.md) authorities.

### Edge Cases (Normative)

- **COINBASE / Fee Recipient:** The COINBASE address **MUST** be included if it experiences any state change. It **MUST NOT** be included for blocks with no transactions, provided there are no other state changes (e.g., from [EIP-4895](../04895.md) withdrawals). If the COINBASE reward is zero, the COINBASE address **MUST** be included as a *read*.
...
An empirical analysis has been done [here](./assets/bal_size_analysis.md). An updated analysis for a 60 million block gas limit can be found [here](./assets/bal_size_analysis_60m.md).
