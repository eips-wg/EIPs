...
Introduce a new instruction that allows smart contracts to create (and update) delegation accounts that match [EIP-7702](./07702.md)'s design. These accounts can be used similarly to [ERC-1167](./01167.md) clones, with significant advantages.
...
Additionally, using EIP-7702 delegations instead of clones/proxies moves the call redirection logic from the userspace to the protocol. This means fewer EVM operations, and thus a reduced gas consumption of every operation that calls these contracts. This includes the verification of signatures and the execution of user operations, but also transfers of assets that include a callback, such as [ERC-721](./00721.md) and [ERC-1155](./01155.md) tokens.

### Scalability

Clones and proxy contracts are present in large numbers in the blockchain state. OpenZeppelin's [ERC-1967](./01967.md) proxies are ~708 bytes. Using EIP-7702 delegations that are only 23 bytes would reduce blockchain state usage.
...
5. add `location` to `accessed_addresses`, as defined in [EIP-2929](./02929.md)
...
Step 9 ensures that an account that was created using `SETDELEGATE` never returns to an empty state (forbiden by [EIP-7523](./07523.md)). Even if the balance is zero, no state was ever set, and the code is removed by a `SETDELEGATE` operation (with zero target), then the nonce being (at least) one would guarantee the account is not empty.
