...
The `FORK_HASH` value as specified in [EIP-6122](./06122.md) of the specific fork, presented as an unsigned `0x`-prefixed hexadecimal number, with zeros left-padded to a four-byte length, in lower case.
...
BPO forks specified in [EIP-7892](./07892.md) should be interpreted as new forks, taking the parent fork as base (recursively, if the parent fork is also a BPO) and updating only the appropriate values in the `blobSchedule` object to produce the new fork configuration.
