...
Any references to `MAX_BLOB_GAS_PER_BLOCK` and `TARGET_BLOB_GAS_PER_BLOCK` in [EIP-4844](./04844.md) should be updated to the new values defined in this EIP when processing the activation block.
The value `BLOB_BASE_FEE_UPDATE_FRACTION_PRAGUE` replaces its previous equivalent when processing the activation block.
These changes imply that `get_base_fee_per_blob_gas` and `calc_excess_blob_gas` functions defined in [EIP-4844](./04844.md) use the new values for the first block of the fork (and for all subsequent blocks).
...
The original target and max values from [EIP-4844](./04844.md) were at a 1:2 ratio. As a consequence, responsiveness to full and empty blob sections was symmetrical:
