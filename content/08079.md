...
To support L1->L2 messages, rollups need to be able to "anchor" information from L1 to the L2. Inspired by [EIP-4788](./04788.md) and existing implementations, we make use of a system transaction to inject a `bytes32 anchor` in the rollup state in a predeploy. This `anchor` can be used to relay L1's state root, a message root, or a rolling hash, or something completely different, based on design preferences.
