...
[EIP-4444](./04444.md) documents the motivation for history expiry itself.
...
Execution layer client MUST implement [EIP-7642](./07642.md) to support the `eth/69` over DevP2P.

Execution layer clients MAY drop pre-merge history according to [EIP-7639](./07639.md).

Consensus layer clients SHOULD NOT depend on Execution Layer clients having the deposit logs from pre-merge blocks and SHOULD implement [EIP-6110](./06110.md).


### Mainnet Activation

Mainnet activation of history expiry will occur shortly (a few days or weeks) after the activation of the [Pectra](./07600.md) hard fork. The short delay is to ensure that all deposit logs from before the fork have been processed before clients begin dropping history.
...
Consensus Layer clients have a dependency on pre-merge deposit logs. [EIP-6110](./06110.md) will remove this dependency when the Pectra fork is activated.
...
The [EIP-7801](./07801.md) DevP2P protocol also provides a peer-to-peer solution for retrieval of this data.
...
Clients of the DevP2P `eth` protocol will need to upgrade to the new `eth/69` version specified in [EIP-7642](./07642.md)

### Pre-Merge Deposit Logs

Consensus Layer clients have had a historical dependency on the deposit logs from pre-merge blocks. Dropping history would make these logs inaccessible to the Consensus Layer client. This issue is mitigated by [EIP-6110](./06110.md)
