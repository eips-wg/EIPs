...
We introduce a new [EIP-2718](./02718.md) transaction, "restricted execution transaction", where the `TransactionType` is `RESTRICTED_EXECUTION_TX_TYPE` and the `TransactionPayload` is the RLP serialization of the following:
...
Wallets will need to be able to understand the meaning of the state changes being permitted by the transaction. This requires wallets to be aware of all major ERC standards, such as [ERC-20](./00020.md) and [ERC-721](./00721.md). In many cases, this also requires the wallets to be able to fetch and analyze the source code of the executed contracts.
